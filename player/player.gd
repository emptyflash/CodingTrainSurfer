...
const LEFT_RAIL = Vector3()

var current_rail = 1 # 0 1 2
...
func _physics_process(delta):
	...
	var movement_vec2 := Input.get_vector(&"move_left", &"move_right", &"move_up", &"move_down")
	#var movement_direction := cam_basis * Vector3(movement_vec2.x, 0, movement_vec2.y)
	#movement_direction.y = 0
	print(position)
	var movement_direction = Vector3(movement_vec2.x, movement_vec2.y, 0.0) + Vector3.FORWARD
